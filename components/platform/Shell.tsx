  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto flex w-full max-w-[1400px]">
        {/* Desktop sidebar */}
        <aside className="sticky top-14 hidden h-[calc(100vh-3.5rem)] w-64 shrink-0 overflow-y-auto border-r border-border/70 bg-background-secondary/40 lg:block">
            ...
            <div className="absolute left-0 top-0 h-full w-72 max-w-[80%] overflow-y-auto border-r border-border bg-background-secondary">
              <div className="flex h-14 items-center justify-between border-b border-border/70 px-4">
                ...
                <button
                  ...
                  aria-label="Close navigation"
                  onClick={() => setOpen(false)}
                  className="grid h-9 w-9 place-items-center rounded-lg border border-border bg-surface text-foreground"
                >
              ...
              <div className="border-t border-border/70 p-4">
                {links.map((link) => {
                  ...
                  const className =
                    "block rounded-lg px-3 py-2 text-sm font-medium text-muted hover:bg-surface-2 hover:text-foreground";
                  return isExternal ? (
                    <a
                      key={link.href}
                      href={link.href}
